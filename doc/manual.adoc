...
WorldEdit is a level editor for Doom 3, forked from DarkRadiant, originally developed by The Dark
Mod team. It may also be used for editing levels for similar *id Software* games
such as Quake 4 and Prey.

This User Guide describes the features and functionality of WorldEdit, however
...
installed into a directory tree which is accessible both by WorldEdit (for
editing) and the game engine itself (for playing the mission).

Once assets have been arranged in WorldEdit according to the wishes of the
...
realtime, making use of the same assets that were accessed by WorldEdit during
editing.

WorldEdit also maintains its own separate file containing various editing
information, such as object layers used within a map. This `.project` file
is never used by the game engine, and is non-critical -- a `.map` file with no
accompanying `.project` file can still be edited with WorldEdit, although
certain WorldEdit-specific state may be lost.
...
which acts as the parent for new brushes and patches created in WorldEdit.
...
When running WorldEdit for the first time, the *Game Setup* dialog will be
shown. This is where you configure the path to the game installation from which
WorldEdit will load assets, such as textures, models and entity definitions.

image::GameSetupDialog.png[align="center"]

The *Game Setup* dialog contains the following options:

*Game Type*:: WorldEdit ships with support for several different game engines,
 each of which is contained within a `.game` file. For editing maps, the default choice is `Doom 3`,
 but it is also possible to edit/create maps for Quake 4 and Prey.

*Mod Base (fs_game_base)*:: This allows a two-level mod structure, where the *Mod Base*
 can point to a major game mod, possibly downloaded from elsewhere, while *Mod*
 can be set to point to an entirely local "sub-mod" which contains local changes and overrides.
...
window without configuring a game installation, in which case WorldEdit will
show a warning and ask if you wish to proceed. If you do, WorldEdit will run
but there will be no available textures, models, entities or other game assets.

=== Interface layout

WorldEdit uses a flexible layout system based on dockable windows which can be moved into
...
WorldEdit defaults to a black-on-white colour scheme in the 2D windows, but ships with
...
Colour schemes only affect what is rendered in the 2D and 3D views by WorldEdit itself.
...
measurement system -- in Doom 3, a typical human stands around 80 - 90
...
IMPORTANT: The fully lit rendering mode in WorldEdit is not identical to what
...
|===
3+^h|Render modes
|image:render_wireframe.png[align="center",width=24]|Render in wireframe mode|
Render objects as wire meshes.
|image:render_solid.png[align="center",width=24]|Render in flat-shaded mode|
Render objects as coloured solids, without textures or lighting.
|image:render_texture.png[align="center",width=24]|Render in fullbright textured mode|
...
|image:media-playback-start-ltr.png[align="center",width=24]|Start render time|
Begin rendering animated effects.
|image:media-playback-stop.png[align="center",width=24]|Stop render time|
Stop rendering animated effects.
3+^h|Far clip options
|image:view_cubicclipping.png[align="center",width=24]|Toggle far clip|
...
TIP: Like other editors in the Radiant family, WorldEdit offers a rather
...
WorldEdit provides six buttons to quickly flip or rotate objects (in 90 degree
...
the system used by WorldEdit may seem somewhat back-to-front. In previous
games, the world starts out as an infinite solid, in which you "carve out" rooms
using subtractive brushes. In WorldEdit, the world starts out as an infinite
...
that way. WorldEdit offers several options for creating multi-sided brushes in
...
WorldEdit provides a couple of shortcuts. These options can be found on the
...
is useful, so it is retained in WorldEdit.
|===
...
. Select a brush.
. Activate *Select Edges* with the toolbar button or *E* key. WorldEdit will
...
Just like with brushes, WorldEdit offers several default patch shapes beyond
...
3 and WorldEdit does not permit the creation of a perfect sphere).
...
When WorldEdit first starts up no textures are loaded and this panel is empty.
...
face. WorldEdit provides the *Surface Inspector* for this purpose, which can
...
order to assist with this, WorldEdit provides a *shader clipboard* which
...
WorldEdit provides certain common functionality to all entities, such as the
...
and patches created inside WorldEdit, or from an external model file in ASE or
...
view and choose *Create model…*. WorldEdit will show the model selector
dialog:
...
2D view and choose *Create player start here*. WorldEdit will create the
player start entity at the clicked position.

Since it makes no sense to have more than one player start location, WorldEdit
...
When you create a model using the <<CreatingModels,model selector>>, WorldEdit
...
primitives created within WorldEdit. In either case, the behaviour of
...
continuously. WorldEdit offers dedicated commands to convert to/from a
...
*TAB* will cause WorldEdit to select a different primitive contained within
...
WorldEdit does not include functionality for compiling a map into the form
needed by the game engine; instead, you must use external tools or the map
compiler built into the game itself.

For Doom 3 and any idtech4 engine base games, the following commands are used within the in-game
...
and WorldEdit to be visible on the screen as separate windows, which you can
...
After the map compilation has failed with a leak, return to WorldEdit and
...
Since a fully developed Doom 3 map is large and complex, WorldEdit provides
...
WorldEdit allows a number of objects to be grouped together, so that they can
...
WorldEdit allows a collection of objects to be saved to disk as a *prefab*,
...
prefabs are a WorldEdit-only feature that have no significance to the engine.
...
WorldEdit provides a limited ability to export scene geometry into a number of
...
frequently-used map elements — WorldEdit is not intended to compete with
...
. Select the geometry to be exported. Note that WorldEdit currently does not
...
objects, WorldEdit also provides a mechanism for controlling the visibility of
...
In addition to the filters supplied with the mod distribution, WorldEdit also
...
become more challenging. To assist with this, WorldEdit provides an *Entity
...
*Save selected as Collision Model...*:: Allows a selected map object (typically
 ...
 After choosing this option, WorldEdit will display the model chooser dialog,
...
*Reload Models/Selected Models/Skins/Scripts/Readable Guis/Materials/Defs/Particles*::
 Forces WorldEdit to re-read the selected category of items from disk,
 refreshing its internal data structures. This is necessary if you have edited
 or re-exported some asset which WorldEdit is using in the current map and you
...
*Exit*:: Exit WorldEdit, prompting to save the current map if necessary.
...
*Make Detail / Make Structural*:: Turns a brush into a func_static, or back to worldspawn
structure.

== Configuring WorldEdit

WorldEdit offers a large number of configurable options which can be used to
...
*Higher selection priority for entities*:: When using the mouse to click on and
 ...
 controls whether WorldEdit will prefer to select the entity (enabled) or the
 brush (disabled).

==== User Interface settings

*Start on Monitor*:: If you have more than one monitor, you can choose which
monitor WorldEdit will start on using this combo box.
...
you to choose which language is used for the WorldEdit interface.

==== Autosave settings

*Enable Autosave*:: Controls whether the autosave feature is active or disabled.

*Autosave Interval*:: Use this slider to choose how often WorldEdit will make
 an autosave. By default, autosaves are made every 5 minutes.

*Save Snapshots*:: If this option is enabled, WorldEdit will make each
 ...
 against WorldEdit or system crashes but not against long-term erroneous
 changes.
...
*Max Snapshot size per map*:: This allows you to control the amount of disk
 ...
 grows above this limit, WorldEdit will start to delete old snapshots.
...
*Ignore light volume bounds when calculating default rotation pivot location*::
 ...
 when multiple objects are selected. WorldEdit will place the rotation widget
...
*Undo Queue Size*:: Use this spinbox to control the number of operations which
 are saved onto WorldEdit's internal undo stack. Larger values provide a
...
*Open last map on startup*:: Enable this option to make WorldEdit
...
WorldEdit is designed to work out of the box with a standard Doom 3 game installation,
however it also offers a few features aimed at mod makers.
...
WorldEdit offers a mechanism to deal with this by allowing certain assets to
...
.assets.lst
----
mymod/chairs/some_broken_chair.lwo=hidden
mymod/tables/another_table.ase=hidden
----
...
WorldEdit. This means that in order to hide specific shaders, you must move
