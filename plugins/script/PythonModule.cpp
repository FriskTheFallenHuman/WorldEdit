...
namespace
{
	constexpr const char* ModuleName = "worldedit";
}
...
{
	rMessage() << "Registering worldedit module to Python using pybind11 version " <<
		PYBIND11_VERSION_STR << std::endl;

	// Register the worldedit module to Python, the InitModule function
...
PyObject* PythonModule::initialiseModule()
{
	try
	{
		...
		_module = py::module::create_extension_module(ModuleName, "WorldEdit Main Module", _moduleDef.get());
