...
/**
 * \brief Base class for many WorldEdit dialogs.
 *
...
public:
    /// Construct and initialise a dialog that is a child of WorldEdit's main window (if present)
    ...
    // dialog will try to be a child to WorldEdit's main window)
    DialogBase(const std::string& title, wxWindow* parent);

    // Construct and initialise a named dialog that is a child of WorldEdit's main window (if present)
    ...
    /**
     * Adjust this window to fit the display WorldEdit is currently (mainly)
