...
#   if defined(ENABLE_RELOCATION)
	return _appPath + "../share/worldedit/";
...
#elif defined(POSIX)
#if defined(ENABLE_RELOCATION)
	return _appPath + "../share/doc/worldedit/";
...
std::string getXDGPath(const std::string& envVar, const std::string& fallback)
{
	...
	if (xdgVar.empty())
	{
		...
		return os::standardPathWithSlash(HOME_DIR) + fallback + "/worldedit/";
	}
	else
	{
...
void ApplicationContextBase::initialise(int argc, char* argv[])
{
	...
	// Construct WorldEdit home directory
	_homePath = appData + "\\WorldEdit";
